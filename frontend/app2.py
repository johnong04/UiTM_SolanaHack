...
st.markdown(
    "<h2 class='section-title'>Available Healthcare Rewards</h2>",
    unsafe_allow_html=True,
)
...
for idx, reward in enumerate(rewards):
    with reward_cols[idx]:
        ...
        if st.button(
            f"Redeem for {reward['points']} points", key=f"redeem_{reward['name']}"
        ):
            redeem_reward(reward["name"], reward["points"])

# Recent Activity Section
st.markdown(
    "<h2 class='section-title'>Recent Healthcare Activities</h2>",
    unsafe_allow_html=True,
)
...
st.markdown(
    "<h2 class='section-title'>Connected Healthcare Platforms</h2>",
    unsafe_allow_html=True,
)
