...
def get_user_balances(public_key):
    """Get user's SOL and token balances"""
    if not public_key:
        return 0, 0
    try:
        balances = solana.check_balance(public_key)
        return balances["sol_balance"], balances["token_balance"]
...
def redeem_reward(reward):
    ...
    _, token_balance = get_user_balances(st.session_state.user_public_key)
    ...
    mnemonic = st.text_input("Enter your mnemonic phrase to sign the transaction:", type="password")
    if not mnemonic:
        return
    ...
    if tx_hash:
        ...
        status_placeholder.error("Transaction failed. Please check your mnemonic and try again.")
        return 0, 0
...
# In your Streamlit app's sidebar section
...
if st.session_state.user_public_key:
    ...
    st.sidebar.write(f"SOL Balance: {sol_balance}")
    st.sidebar.write(f"Token Balance: {token_balance}")
